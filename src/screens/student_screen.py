...
def student_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout",type='secondary', key='student_logout_button',shortcut="control+backspace"):
...
def student_screen():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go back to Home",type='secondary', key='student_backbtn',shortcut="control+backspace"):
            st.session_state['login_type'] = None
            st.rerun()
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name",placeholder='E.g. Shaurya Mishra')
            ...
            try:
                audio_data = st.audio_input('Record a short note like I am present, My name is Shaurya Mishra')
